...
void loop() {
  delay(10);
  b1_state = digitalRead(PUSH1);
  debounce1();
  b2_state = digitalRead(PUSH2);
  debounce2();
  
  // put your main code here, to run repeatedly: 
  if(digitalRead(b1_state) == LOW){
  Serial2.write('1');
  Serial.print('1');
  }
  else if(digitalRead(b2_state) == LOW){
  Serial2.write('2');
  Serial.print('2');
  }
}
...
void debounce2(void) {
  if (b2_state == LOW)     //Asks if the push is pressed
  {
    d2_flag = 2;     //Variable changes value
  }
  if (b2_state == HIGH and d2_flag == 1)
  {

    //Makes the desired action
    d2_flag = 0;    //Variable turns to the original value
  }
}*/
